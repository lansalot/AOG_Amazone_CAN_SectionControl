...
const int SPI_CS_PIN = 10;   // ATTENTION !! PIN9 is also used already in Machine USB but is PIN9 or 10 ar needed for CAN Shield
...
void setup()
{
  ...
  { // init can bus: Amatron baudrate = 250k
...
void loop()
{
  ...
  if (Serial.available() > dataLength && isHeaderFound && isPGNFound)
  {
    ...
    else if (pgn == 0xEE) // EE Machine Settings
    {
    ...
    else if (pgn == 0xEC) // EC Relay Pin Settings
    {
...
void getSectionInformationfromAOG(void)
{
  ...
  // check if minimum one of the section relay is on/activ -> if so Main switch in Amatron on/off // ANDREW LOOK AT THIS TOO
...
{
  bitWrite(send_AOG_sectionsbyte9_on, sectionIndex, state);
  bitWrite(send_AOG_sectionsbyte10_off, sectionIndex, !state);
  delay(1);
}
  ...
  if (CAN_MSGAVAIL == CAN.checkReceive()) // check if data coming
  {
    ...
    { // ID for sections info is 1CE7268.
      ...
      if (buf[3] == 1 || buf[3] == 0)
      {
        ...
        {
        case 0xf9:
          switchSection(0, buf[3]);
          break;
        case 0xfd:
          switchSection(1, buf[3]);
          break;
        case 0x01:
          switchSection(2, buf[3]);
          break;
        case 0x05:
          switchSection(3, buf[3]);
          break;
        case 0x09:
          switchSection(4, buf[3]);
          break;
        case 0x0d:
          switchSection(5, buf[3]);
          break;
        case 0x11:
          switchSection(6, buf[3]);
          break;
        }
      }
    }
